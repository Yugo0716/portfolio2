...
export default async function WorksSection() {
  ...
  const pickUps = works.filter((w: Works) => w.isPickUp);
  const rests = works.filter((w: Works) => !w.isPickUp);
  ...
  return (
    <section className={styles.section}>
      ...
      <h3 className={styles.subheading}>Pick Up</h3>
      {renderWorks(pickUps)}

      <h3 className={styles.subheading}>Others</h3>
      {renderWorks(rests)}
    </section>
  );
}
